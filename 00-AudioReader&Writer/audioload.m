...
%   in a structure to be used later on. File structure will be necessary
%   for audiosave() function.
%
...
if ~isempty(strfind(type1, file.ext))
	...
	finfo = audioinfo([PathName FileName]);
	file.nbit = finfo.BitsPerSample;
...
[file.len, file.ch] = size(file.data);

end

%%%%%%%%%%%%%%%%%% External functions to read mp3 format %%%%%%%%%%%%%%%%%
function [Y,FS,RATE,NBITS,OPTS] = mp3read(FILE,N,MONO,DOWNSAMP,DELAY)
...
% 2016-02-02 Added bitrate reader as input.
...
MPG123059 = 0;
mpg123 = fullfile(path,['/mp3bin/mpg123.',ext]);
mp3info = fullfile(path,['/mp3bin/mp3info.',ext]);
